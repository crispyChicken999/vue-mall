<template>
    <div class="item-detail" v-if="this.$store.state.goodStatus">
        ...
        <div class="button">
            <div class="add-to-cart" @click="enterCartComponent">加入购物车</div>
...
export default {
    ...
    methods:{
        exitDetailPages(){
            ...
            if(!this.$store.state.goodStatus){
        ...
        buynow(item){
            ...
            if(index!=-1){
                this.$store.state.cartItem[index].num +=1;
            }else{
                this.$store.state.cartItem.push(obj);
            }
